...
the Intel SDM meaning and the Linux kernel meaning. The goal was to document the differences and
explain the reasons behind them, helping Linux power management developers navigate the complexity
and historical baggage behind the term.
...
- [Intel CPU Base Frequency Explained: Definitions and Performance Scaling Fundamentals](#intel-cpu-base-frequency-explained-definitions-and-performance-scaling-fundamentals)
  ...
  - [Linux Kernel](#linux-kernel)
    ...
    - [The intel\_pstate Driver](#the-intel_pstate-driver)
...
While Intel still advertises base frequency for server and some desktop client CPUs with only
P-cores, it stopped doing so for mobile client CPUs starting with
...
- **Fixed Base Frequency**: CPUID.16H or MSR_PLATFORM_INFO definition, it is simply the default
  ...
  Probably not very useful for software.
