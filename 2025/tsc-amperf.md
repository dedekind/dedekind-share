...
- [TSC, APERF, and MPERF Counters](#tsc-aperf-and-mperf-counters)
...
timer subsystem uses the CPU's LAPIC controller in TSC deadline mode, which allows timer
interrupts to be armed by specifying the target TSC value at which to trigger them.
...
to help distinguish between the two counters.
...
- APERF and MPERF increment only while the CPU is in C0.
  - If a logical CPU executes HLT or enters a C-state via MWAIT, both APERF and MPERF stop
    incrementing.
...
While TSC operates based on the crystal clock frequency, APERF and MPERF use a fixed reference
frequency that tracks CPU performance. The bus clock frequency varies to reduce electromagnetic
interference, which helps explain why MPERF frequency is close to TSC frequency, but not identical.
