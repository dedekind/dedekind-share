...
- [C-state Namespaces](#c-state-namespaces)
...
This document discusses C-states on Intel platforms and focuses on classifying them by namespace.
...
- **intel_idle (native mode):** Uses built-in, platform-specific C-state tables to determine
...
- **C1_ACPI**: Mapped to the requestable C1 (`MWAIT 0x0`). However, due to the "C1E autopromote"
...
- **C2_ACPI**: Mapped to the requestable C6P (`MWAIT 0x21`). However, since the package C-state
  limit is set to PC0, PC6 is disabled. As a result, the deepest hardware C-state for C6P will be
  CC6 (core C6).
