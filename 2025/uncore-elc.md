...
- [Intel Xeon Uncore ELC (Efficiency Latency Control)](#intel-xeon-uncore-elc-efficiency-latency-control)
...
for each die type: 800MHz minimum for all dies, 2.2GHz maximum for compute dies, and 2.5GHz
maximum for I/O dies.

The ELC low threshold is 11%, and the ELC high threshold is 95%. The ELC low and middle zone
minimum uncore frequencies are 1.2GHz for compute dies, and 800MHz for I/O dies.
...
Let's set the ELC high threshold to 50%. Because the ELC high threshold is already enabled, this
ensures the processor enters the ELC high zone.
